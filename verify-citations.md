...
当输入为项目申请书等**不含正文引用角标**的文件时，断言提取方式不同，但**验证标准和判定逻辑不变**——核心任务始终是检验断言准确性：
...
|------|----------------|-------------------|
| 文档转换 | 第一步：多格式→MD | 同 |
| 断言提取 | 正则匹配 `[n]` 角标 | **人工阅读**立项依据等关键段落，识别包含事实性声明的句子，追溯其引用的文献 |
| 文献存在性检查 | 属于验证的一部分 | 同（断言验证的前提条件） |
| 验证方法 | PubMed API + WebSearch | 同 |
| **判定标准** | **断言准确性** | **断言准确性**（不变！） |
| 报告输出 | HTML 报告存桌面 | 同 |

### 申请书断言提取指南

项目申请书的断言通常集中在**立项依据**部分，形式为"事实陈述 + 文献编号"，而非正式论文的角标。例如：

> "我国成年人牙周炎的患病率高达85%[3]" → 需验证[3]是否确实报告了85%的患病率
> "57.1%的牙周炎患者在初始治疗后失访[5]" → 需验证[5]中是否确有57.1%这个数据

**提取原则：**
1. 优先从立项依据、研究现状部分提取含事实性数据的句子
2. 每个数字、百分比、因果关系的陈述都需要对应文献支撑
3. 与用户确认断言列表后再开始验证

### 局限性

- 申请书断言提取依赖人工阅读，可能遗漏或误判
- 中文文献 WebSearch 覆盖面不及 PubMed API，负面结果需注明"基于公开网页搜索，可能漏检付费墙内文献"
- 无论哪种输入格式，最终报告均需注明"基于摘要验证，可能需查全文确认"
