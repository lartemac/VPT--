...
- 🔧 **电脑卡顿问题诊断与解决（讯飞输入法残留）**
  ...
  - **创建工具**：
    - `系统清理.bat` - 自动清理启动项和临时文件
    - `系统诊断和清理工具.py` - Python版诊断工具
    - `电脑卡顿问题解决方案.txt` - 详细解决指南

  - **语音输入替代方案推荐**：
    - ✅ Windows内置语音识别（Win + H）
    - ✅ 微软PPT语音转文字
    - ✅ 腾讯云在线语音识别（免费）
    - ❌ 不推荐其他国产输入法（可能也有资源占用问题）
...
- 🔧 **API服务使用规范制定与工具替代方案**
  - **时间**：2026-02-01
  - **系统**：Windows 11
  - **背景**：MCP服务次数每月刷新，需要优化使用策略

  - **新规范：MCP和WebSearch使用限制**
    - ❌ **禁止直接调用**：MCP服务和WebSearch
    - ✅ **必须先询问**：用户是否同意调用
    - 💡 **目的**：节约有限的API调用次数
  ...
  - **语音输入需求调研**：
    ...
    - **特点**：
      ...
      - 完全免费
      - 医学专业词库
    - **下载地址**：https://srf.xunfei.cn/

  - **重要规则更新**：
    - **资源节约原则**：优先使用本地工具，避免不必要的API调用
...
- ✅ **Windows平台医学统计图表复现与优化**
  - **完成时间**：2026-01-29
  - **数据来源**：急性牙髓炎15例最终数据20251126（最终）.xlsx
  - **项目背景**：用户需要在Windows系统复现Mac平台绘制的医学统计图表

  - **图1复现：各时间点VAS变化情况图**
    - **数据提取**：从Excel第11、31、33、35、37、41列（0-based索引）
      - 术前VAS（列11）：均值6.47
      - 术后24h（列31）：均值1.13
      - 统计检验：t=7.1967, P=0.000005
    - **绘图脚本**：`图1绘制.py`（保存在桌面）
    - **图表规格**：
      - 散点：灰色（#808080），s=50，双向抖动（x±0.12, y±0.15）
      - 均值连线：红色（#FF0000），linewidth=2.5
      - 箱线图：仅在术前和术后24h，宽度0.17（重要：原来的1/3）
      - Y轴范围：0-9（而非0-10）
      - Y轴刻度：0/2.5/5.0/7.5（不标记9）
      - ❌ 无图例（删除"均值"图例）
      - ❌ 无网格线（学术图表规范）
      - 分辨率：300 DPI
      - 文件：`图1复现.png`
    - **用户修改要点**：
      1. 箱线图宽度缩减到现在的三分之一（0.5→0.17）
      2. 删除右上角"均值"图例
      3. Y轴到9即可，不必到10
      4. 删除象限内的虚线（网格线）
      5. 清理无用.py文件
  ...
  - **图3处理：手动删除网格线**
    - **问题**：原始Mac脚本生成的图3.png包含网格线（虚线）
    - **解决方案**：用户使用Photoshop手动删除网格线
    - **重要发现**：❌ 学术图表不应包含网格线/虚线装饰元素
    - **修改文件**：图3复现.png（用户手动处理）

  - **图3复现尝试（失败后放弃）**
    - **初始尝试**：从Excel原始数据复现图3（5幅子图，临床参数相关性分析）
    - **遇到问题**：数据来源混淆（Word vs Excel）
    - **最终方案**：用户要求使用Word文档数据，但Word只包含汇总统计（均值±标准差），无原始数据
    - **决定**：❌ 删除图3复现.png和图3绘制.py，等待用户提供完整绘图命令

  - **PubMed医学文献查询系统开发**
    - **项目需求**：用户需要查询医学文献并获取DOI，用于学术论文引用验证
    - **创建脚本**：
      1. `pubmed_search_fixed.py` - 牙髓炎疼痛管理相关查询（72篇文献）
      2. `pubmed_search_anesthesia.py` - 阻滞麻醉vs局部麻醉对比查询（104篇文献）
      3. `PUBMEDCONSULT.py` - **通用PubMed查询系统**（重点交付）
    - **PUBMEDCONSULT.py 特点**：
      - ✅ 支持中英文查询语句（自动翻译关键词）
      - ✅ 智能生成8个搜索关键词
      - ✅ 按相关性自动排序（智能评分系统）
      - ✅ 限制每次查询12篇最相关文献
      - ✅ 自动保存到桌面：`[语句概要]_pubmed查询.xlsx`
      - ✅ 包含PMID、DOI、标题、作者、期刊、年份、链接
      - ✅ 使用官方PubMed E-utilities API
      - ✅ 遵守API请求限制（每秒最多3次）
    - **相关性评分系统**：
      - 系统评价/Meta分析：+8分
      - RCT随机对照试验：+5分
      ...
      - 高影响期刊：+2分
    - **使用方法**：
      - 方法一：交互式输入 `python PUBMEDCONSULT.py`
      - 方法二：命令行参数 `python PUBMEDCONSULT.py "查询语句"`
    - **测试结果**：
      - 查询："pulpitis pain management anesthesia"
      - 结果：找到91篇，筛选12篇，100%有DOI
      - 文件：`pulpitis_pain_manage_pubmed查询.xlsx`
    - **文件位置**：`C:\Users\Administrator\Desktop\PUBMEDCONSULT.py`
    - **配套文档**：`PUBMED使用说明.txt`（详细使用指南）
  ...
  - **重要经验教训**：
    ...
    **1. 学术图表绘制规范** ⭐⭐⭐
    - ❌ **禁止在学术图表中画网格线/虚线**
      ...
      - 原因：网格线会被视为装饰性元素，降低专业性和可读性
      - 例外：除非用户明确要求，否则默认ax.grid(False)
    ...
    - **2. 平台路径区分的重要性** ⭐
    - **问题**：多次引用Mac路径导致脚本在Windows系统上失败
    - **Mac路径示例**：`~/Desktop/副本急性牙髓炎15例最终数据20251122.xlsx`
    - **Windows路径**：`C:\Users\Administrator\Desktop\急性牙髓炎15例最终数据20251126（最终）.xlsx`
    - **解决方案**：
      - 检测`Platform`环境变量（win32 vs darwin）
      - Windows路径：`D:\cc-github\`，分隔符`\`
      - macOS路径：`~/cc-github/`，分隔符`/`
    - **未来规则**：
      - 所有新脚本必须标注平台和创建时间
      - 跨平台脚本必须使用条件判断区分路径

    - **3. 脚本标注规范** ⭐
    - **要求**：未来生成任何Python脚本时都要标记好时间和所在系统
    - **格式**：
      ```python
      """
      脚本功能描述
    ...
    - **4. 图表元素美观和规范** ⭐⭐
    - **问题**：早期尝试复现图3时，元素分布不紧凑、不美观
    - **用户要求**：
      - 图例、标题、坐标轴描述等元素分布要规范和紧凑
      - 箱线图要美观（粗细适中、位置准确）
      - 散点分布要自然（抖动规则合理）
    - **具体标准**：
      - **标题**：字体大小适中，位置不拥挤
      - **图例**：仅在有需要时显示，避免遮挡数据
      - **坐标轴**：标签清晰、刻度合理（不过密）
      - **箱线图**：宽度0.17，边框1.5，中位数线2.0
      - **散点**：双向抖动，避免重叠
      - **配色**：统一使用淡蓝色+灰色+红色方案

    - **5. 用户沟通要点**
    - **用户反馈**："我感觉你越改越乱，而且现在文件夹里有一堆没用的.bat"
    - **用户偏好**：简洁、高效、无冗余文件
    - **改进措施**：
      - 及时清理临时文件
      - 不要创建过多测试脚本
    ...
    - **6. 数据来源确认**
    - **问题**：多次混淆数据来源（Word文档 vs Excel表格）
    - **Word文档特点**：包含汇总统计（均值±标准差、P值），无原始个体数据
    - **Excel表格特点**：包含原始个体数据，适合绘制箱线图和散点图
    - **规则**：绘图使用Excel原始数据，统计分析参考Word文档

    - **7. PubMed API使用技巧**
    - **esearch接口**：搜索文献，返回PMID列表
    - **esummary接口**：获取基本信息（标题、作者、期刊、DOI）
    - **efetch接口**：获取完整摘要（Background, Methods, Results, Conclusions）
    - **请求限制**：每秒最多3次请求（遵守0.34秒间隔）
    - **排序**：使用`sort='relevance'`按相关性排序
    - **去重**：基于PMID去重，避免重复文献

    - **8. Polymath模式激活**
    - **触发关键词**：牙髓炎、VPT、活髓保存、文献检索、PubMed
    - **激活模式**：Mode C（医学科研）
    - **专业能力**：
      - 医学问答和诊断标准
      - 科研论文和标书撰写
    ...
    - **示例**：
      - `图1绘制.py` - 清晰表明用途
      - `PUBMEDCONSULT.PY` - 全大写，突出重要性
      - `pubmed_search_fixed.py` - 描述性命名
    - **避免**：test_、temp_、debug_等临时性前缀

    - **10. Excel文件命名**
    - **规则**：`[语句概要]_pubmed查询.xlsx`
    - **实现**：自动提取查询语句前20字符作为文件名
    - **特殊字符处理**：替换空格和特殊字符为下划线
    - **示例**：`pulpitis_pain_manage_pubmed查询.xlsx`

  - **清理工作**
    - ❌ 删除临时测试脚本：
      - pubmed_search*.py（所有之前的测试版本）
      - google_scholar_search.py
      - analyze_anesthesia_papers.py
      - check_word_data.py
      - check_word_full_data.py
      - extract_word_data.py
      - pubmed_debug.xml
      - pubmed_search_log.txt
    - ✅ 保留最终脚本：
      - `PUBMEDCONSULT.py`（通用查询系统）
      - `图1绘制.py`（图1复现脚本）
      - `PUBMED使用说明.txt`（使用指南）
  ...
  - **Git同步计划**
    - 待提交到GitHub：
      - `PUBMEDCONSULT.py`（新增）
      - `PUBMED使用说明.txt`（新增）
      - `memory.md`（更新2026-01-29内容）
    - 提交信息：
      - 新增Windows平台医学统计图表复现工作
      - 新增PubMed通用查询系统
      - 更新重要经验和规范

  - **用户价值**：
    - ✅ 掌握Windows平台医学统计图表绘制方法
    - ✅ 建立学术图表绘制规范（无网格线、箱线图宽度0.17）
    - ✅ 获得强大的PubMed文献查询工具（支持中英文）
    - ✅ 学会跨平台开发注意事项（路径区分、平台检测）
    - ✅ 明确脚本标注规范（时间、系统、功能）
    - ✅ 优化图表元素美观和紧凑布局

  - **状态**：✅ 图1复现已完成，图3待用户提供完整绘图命令
  - **下一步**：
    - 等待用户提供图3的完整绘图要求
    - 将工作成果提交到GitHub
    - 更新Polymath Skills（医学科研模块绘图规范）

### 2026-01-28
- ✅ **AI API 智能模型自动选择系统建立（GLM-4.7）**
  - **建立时间**：2026-01-28
  - **核心原则**：Claude 根据任务类型自动选择最优模型，用户无需手动指定
  - **配置文件**：
    - 全局配置：`~/.claude/CLAUDE.md`（已更新）
    - GLM-4.7 助手：`~/Desktop/VPT-初诊数据/glm47_helper.py`
  ...
  - **API 优先级**：
    | 任务类型 | 首选 API | 备选 API |
    |---------|---------|---------|
    | 医学问答 | GLM-4.7 Plus | Claude (医学 Skill) |
    | 代码开发 | Claude | GLM-4.7 Flash |
    | 文献搜索 | DuckDuckGo | GLM-4 Web Search |
    | 数据分析 | GLM-4.7 Plus | 本地 Python |
    | 网络搜索 | DuckDuckGo | Claude WebSearch |

  - **API 配置**：
    - API Key：REDACTED_GLM_API_KEY
    - SDK：zhipuai（已安装并验证）
    - 模型：glm-4-plus, glm-4-flash, glm-4-air, glm-4
    - 状态：✅ 已验证可用

  - **适用范围**：此规则适用于所有 AI API（GLM、Claude、Gemini 等），未来新增 API 也需遵守
...
- ✅ **Mac 平台三张医学统计图表重现制作**
  - **完成时间**：2026-01-28
  - **数据来源**：副本急性牙髓炎15例最终数据20251122.xlsx
  - **生成图表**：
    - 图1-5.png (212 KB) - 各时间点 VAS 变化情况图
    - 图2.png (252 KB) - 术前 VAS 及年龄与术后24h VAS 相关性分析（3幅子图）
    - 图3.png (240 KB) - 临床参数与术后24h VAS 相关性分析（5幅子图）
  - **特点**：300 DPI 高分辨率，Arial Unicode MS 字体（Mac 适配）
  - **保存位置**：~/Desktop/

- ✅ **急性牙髓炎15例数据医学统计图表绘制项目**
  - **项目时间**：2026-01-28
  - **数据来源**：急性牙髓炎15例最终数据20251126（最终）.xlsx
  - **研究主题**：VPT（活髓保存治疗）疗效评估，术后VAS疼痛评分变化
  - **完成图表**：
    1. **图1-5**：各时间点VAS变化情况图
    2. **图2**：术前VAS评分及年龄与术后24小时VAS评分的相关性分析（3幅子图）
    3. **图3**：人口学特征及临床参数与术后24小时VAS的相关性分析（5幅子图）

  - **确定的医学统计图表风格标准**：
    - **字体设置**：
      - 中文：宋体加粗（SimSun）
      - 英文和数字：Times New Roman加粗
    - **配色方案**：
      - 散点：灰色（#808080），alpha=0.6，边缘深灰色
      - 均值连线：红色（#FF0000），linewidth=2.5
      - 箱线图填充：淡蓝色（lightblue），alpha=0.7
      - 箱线图边框：蓝色（blue），linewidth=1.5
      - 中位数线：深蓝色（darkblue），linewidth=2
      - 工字线/误差条：蓝色（blue），linewidth=1.5
      - 网格线：虚线（--），alpha=0.3，linewidth=0.5
    - **线条宽度标准**：
      - 普通线（网格、边框）：1.5
      - 重要线（中位数、箱线图边框）：2
      - 强调线（均值连线、P值线）：2.5
    - **散点样式**：
      - 颜色：灰色，s=50
      - 抖动：水平方向±0.12，竖直方向±0.15
      - 边缘：深灰色，linewidth=0.5
      - 透明度：alpha=0.6
    - **箱线图样式**：
      - 标准箱线图（图1、图2）：matplotlib默认（Q1-Q3，1.5×IQR）
      - 特殊箱线图（图3）：箱体=[0,均值]，工字线=均值±SE（标准误）
      - 箱体宽度：0.17
      - 工字线宽度：箱体宽度的2/3（0.113）
    - **边框设置**：
      - 删除顶部和右侧边框（spines['top'].set_visible(False)）
      - 保留左侧和底部边框，linewidth=1.5
    - **图表尺寸**：12×8英寸（图1、图2），16×11英寸（图3）
    - **分辨率**：300 DPI
    - **P值标注**：Times New Roman，fontweight='bold'，fontsize=14

  - **图1-5：各时间点VAS变化情况图**
    - **数据结构**：
      - 横轴：术前、术后24h、术后3天、术后7天、术后1个月、术后6个月
      - 纵轴：VAS评分（0, 2.5, 5.0, 7.5）
      - 样本量：15例
    - **图表元素**：
      - 箱线图：术前和术后24h（标准Q1-Q3箱线图）
      - 散点：所有时间点（灰色，双向抖动）
      - 均值连线：红色，连接6个时间点
      - P值标注：P < 0.001（配对t检验）
    - **统计结果**：
      - 术前 vs 术后24h：t=7.1967, P=0.000005
      - 术前均值：6.47，术后24h均值：1.13
    - **文件保存**：桌面\图1-5.png

  - **图2：相关性分析图（术前VAS及年龄 vs 术后24h VAS）**
    - **数据**：Spearman相关系数
    - **图表布局**：1行3列
    - **子图A**：术前VAS vs 术后24h VAS
      - 横轴：术前（4-9），纵轴：术后（-2到4）
      - Spearman R=-0.509, P=0.053
    - **子图B（中）**：年龄 vs 术前VAS
      - 横轴：年龄（20, 30, 40, 50, 60），纵轴：VAS（4-9）
      - Spearman R=-0.123, P=0.662
    - **子图B（右）**：年龄 vs 术后24h VAS
      - 横轴：年龄（20, 30, 40, 50, 60），纵轴：VAS（-2到4）
      - Spearman R=-0.384, P=0.158
    - **图表元素**：
      - 散点：灰色
      - 回归线：深蓝色，linewidth=2
      - 置信区间：浅蓝色，95%置信区间
  ...
  - **图3：临床参数与术后24h VAS相关性分析（5幅子图）**
    - **数据来源**：完整Excel文件（49列），包含临床参数
    - **图表布局**：2行3列（上3下2），删除右下角子图
    - **分析参数**：
      1. 咬合痛（否 vs 是）
      2. 根尖周围透亮区（PAI 1 vs PAI 2）
      3. 继发龋（否 vs 是）- 原数据"充填物"
      4. 麻醉方式（局部 vs 阻滞）
      5. 近髓处探诊质地（硬 vs 软）
    - **箱线图样式（特殊设计）**：
      - 箱体：从0到均值（非Q1-Q3）
      ...
      - 无延伸须线
    - **关键统计量验证**：
      - 咬合痛-否：均值=1.31，工字线=[0.80, 1.82]
      - 根尖周围透亮区-PAI 1：均值=1.55，工字线=[0.97, 2.12]
      - 继发龋-否：均值=1.55，工字线=[0.97, 2.12]
      - 麻醉方式-局部：均值=1.89，工字线=[1.23, 2.54]
      - 近髓处探诊质地-软：均值=1.42，工字线=[0.88, 1.96]
    - **图表元素**：
      - 箱线图：只在指定组绘制（咬合痛-否、PAI 1、充填物-无、麻醉-局部、探诊质地-软）
      - 散点：两组都有（灰色，双向抖动）
      - 显著性标记：所有图都有星号标注（P<0.05）
      - 工字线宽度：箱体宽度的2/3
    - **文件保存**：桌面\图3.png（用户已重命名）

  - **绘图工具栈**：
    - Python 3.14
    - pandas：数据读取和处理
    - matplotlib：图表绘制（3.10.8）
    - numpy：数值计算和数组操作
    - scipy.stats：统计检验（t检验、Spearman相关分析）
    - openpyxl：Excel文件读取

  - **经验总结**：
    1. **箱线图类型选择**：
       - 时间序列数据（图1）：使用标准Q1-Q3箱线图，符合统计学规范
       - 组间比较（图3）：使用均值±SE特殊箱线图，符合特定领域惯例
    2. **字体设置**：matplotlib中fontname='SimSun'（中文）和family='Times New Roman'（英文）
    3. **散点抖动**：双向抖动（水平+竖直）使分布更自然，避免重叠
    4. **P值标注位置**：需要在箱线图上方预留足够空间，避免重叠
    5. **配色一致性**：所有图表使用统一配色方案，提升专业性和一致性
    6. **图表尺寸**：根据子图数量合理调整，确保可读性

  - **用户价值**：
    - ✅ 完成医学论文所需的3张高质量统计图表
    - ✅ 符合医学期刊发表标准
    - ✅ 确定统一的图表风格，便于后续使用
    - ✅ 所有图表使用300 DPI高分辨率，适合印刷

  - **状态**：✅ 已完成，用户确认满意
  - **下一步**：将图表风格标准添加到 Polymath Skills 的医学科研模块

### 2026-01-13
- ✅ **搜索方案探索：DuckDuckGo - 完美解决 Claude WebSearch 月度限制问题**
  - **探索时间**：2026-01-13 全天
  - **核心需求**：寻找可靠的搜索引擎替代 Claude WebSearch（月度使用上限）
  - **关键要求**：搜索结果必须来自**真实互联网**，而非 AI 训练数据

  - **尝试方案1：GLM-4 API web_search**
    - **测试结果**：❌ 不可用
    - **问题发现**：
      - 不稳定（有时返回结果，有时不返回）
      - 无法验证是否实时网络搜索（可能是训练数据）
      - 测试搜索"2026年1月13日新闻"返回"无法搜索实时新闻"
    - **结论**：不推荐使用，无法满足用户要求

  - **尝试方案2：DuckDuckGo Search**
    - **测试结果**：✅ 完美通过
    - **验证测试**：
      - ✅ 中文搜索："中信银行留学信用卡年费" → 找到3条结果
      - ✅ 英文搜索："Python 3.14 new features" → 找到Python官网发布
      - ✅ 新闻搜索："人工智能新闻 2026" → 找到2026-01-04的新闻（实时性验证！）
      - ✅ 实时性验证：包含未来日期（2026年），确认是真实网络搜索，非训练数据
    - **核心优势**：
      - ✅ 完全免费，无需 API key
      - ✅ 无使用限制（无限次）
      - ✅ 保护隐私（不追踪用户）
      ...
      - ✅ 中英文都支持良好
      - ✅ 基于 Bing，稳定可靠
    - **结论**：强烈推荐，作为 Claude WebSearch 的完美补充

  - **已创建的工具**：
    - `ddg_search.py` - 主搜索工具（支持普通搜索和新闻搜索）
    - `test_ddgs.py` - 测试脚本（验证搜索功能）
    - `DDG_SEARCH_GUIDE.md` - 完整使用指南
    - `SEARCH_SOLUTION_SUMMARY.md` - 完整探索总结

  - **全局配置更新**：
    - 更新 `~/.claude/CLAUDE.md`
    - 新增规则：优先使用 DuckDuckGo 搜索
    - 搜索策略：
      - 中英文日常信息 → DuckDuckGo（免费、无限）
      - 英文学术文献 → Claude WebSearch（质量高）
      - Claude 达到上限 → DuckDuckGo（备用方案）

  - **其他 API 调研**：
    - **Gemini 3.0 API**：
      - 最新版本：Gemini 3.0（2025年11月发布）
      - 新功能：思维控制、媒体分辨率提升、智能代理
      - 定价：按使用量计费，无月度搜索限制
      - 支付方式：接受 Visa 信用卡
    - **中信银行 Visa 卡支付**：
      - ✅ 理论上可用于 Gemini API 订阅
      - 条件：必须是信用卡（非借记卡），已开通境外支付
      - 实际可用性：需测试验证
